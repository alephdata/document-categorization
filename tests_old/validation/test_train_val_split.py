from preprocessing.train_val_test_split import custom_train_val_test_split
import pandas as pd
import pytest
...
def test_custom_train_val_test_split(iris_df):
...
def test_custom_train_val_test_split_random_seed(iris_df):
...
def test_custom_train_val_test_split_oversampling(iris_df):
...
def test_custom_train_val_test_split_undersampling(iris_df):
    train_df, val_df, test_df = custom_train_val_test_split(iris_df, class_col="variety", random_seed=77)
    (
...
def test_train_val_test_disjoint(iris_df):
