...
from prediction.predict import (load_classifiers, predict_documents,
                                predict_from_directory, predict_image,
                                predict_images, predict_pdf, predict_pdfs,
                                predict_tiff, predict_tiffs)
...
@pytest.fixture
def document_path_pagecount(jpg_image_path: List[str],
                            pdf_path_pagecount: List[Tuple[str, int]],
                            tiff_path_pagecount: List[Tuple[str, int]]) -> List[Tuple[str, int]]:
...
def documents_folder_path_pagecounts(
        tmp_path: Path,
        document_path_pagecount: List[Tuple[str, int]]
...
def test_load_classifiers(model_name: str) -> None:
    ...
    # check types
    assert isinstance(binary_clf, FirstpageClassifier)
...
def test_predict_from_directory(model_name: str,
                                documents_folder_path_pagecounts: Tuple[str, List[Tuple[str, int]]]) -> None:
    directory, documents_path_pagecount = documents_folder_path_pagecounts

    page_counts = {
        os.path.basename(file_path): page_count
        for file_path, page_count in documents_path_pagecount
    }
